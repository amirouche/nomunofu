...
(library (arew data base lsm)
  ...
  (define (call-with-lock obj thunk)
    ...
    (define lsm (load-shared-object "./local/lib/lsm.so"))
    ...
    (define-syntax-rule (check code)
      (let ((code* code))
        (unless (zero? code*)
          (error 'lsm (error->message code*) code*))))
