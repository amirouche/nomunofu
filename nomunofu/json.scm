...
;;
(define-module (nomunofu json))

(import (scheme base))
(import (scheme list))
(import (scheme cxr))
(import (scheme charset))
(import (nomunofu combinator))
...
(define-public (json-null? object)
  (eq? object json-null))
...
(define (%json->scheme string)
  ...
  ;; 2) Parse stream of xchar, to a stream of tokens removing useless space.
  ;; 3) Parse tokens into a scheme object
  (parse json (list->stream (parse tokens (string->stream string)))))

(define-public (json->scheme string)
  (guard (ex (else #f))
    (%json->scheme string)))
